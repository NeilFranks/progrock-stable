...
def dynamic_value(incoming):
    # we only need to do something if it's a string...
    ...
    try:
        subprocess.run(
            ['realesrgan-ncnn-vulkan', '-i',
                '_esrgan_orig.png', '-o', '_esrgan_.png'],
...
def do_gobig(gobig_init, gobig_scale, device, model, opt):
    ...
    if opt.gobig_maximize == True:
        ...
        target_image = input_image.resize(
            new_canvas_size, get_resampling_mode())
        slices, new_canvas_size = grid_slice(
            target_image, overlap, (opt.W, opt.H))
    input_image.close()
